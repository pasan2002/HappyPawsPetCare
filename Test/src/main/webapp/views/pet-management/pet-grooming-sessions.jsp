...
<head>
    ...
    <title>Grooming History — <%= pet != null ? pet.getName() : "Pet" %> — Happy Paws PetCare</title>
    <meta name="description" content="View grooming history for your pet." />
...
<section class="relative overflow-hidden">
    <div class="absolute inset-0 bg-gradient-to-b from-purple-50 via-white to-white dark:from-slate-900 dark:via-slate-950 dark:to-slate-950"></div>
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
        <div class="reveal">
            ...
            <h1 class="font-display text-3xl md:text-4xl font-extrabold tracking-tight">
                Grooming History for <%= pet != null ? pet.getName() : "Pet" %>
            </h1>
            <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">
                View your pet's complete grooming history
            </p>
        </div>
    </div>
</section>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
    ...
    <div class="reveal">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold">Grooming History</h2>
                ...
                <div class="flex items-start justify-between mb-4">
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            <span class="inline-flex items-center px-2 py-1 rounded-lg text-xs font-medium bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">
                                Grooming Session
                            </span>
                            <span class="text-sm text-slate-500">
                                <%= groomingSession.getSessionTime() != null ? groomingSession.getSessionTime().format(dateFormatter) : "Date not specified" %>
                            </span>
                        </div>
                        <h3 class="text-lg font-semibold">
                            Groomer: <%= groomingSession.getStaffName() != null ? groomingSession.getStaffName() : "Unknown" %>
                        </h3>
                    </div>
                    <% if (groomingSession.getServiceType() != null && !groomingSession.getServiceType().trim().isEmpty()) { %>
                    <div class="text-right">
                        <div class="text-sm text-slate-500">Service Type</div>
                        <div class="text-lg font-semibold capitalize"><%= groomingSession.getServiceType() %></div>
                    </div>
                    <% } %>
                </div>

                <% if (groomingSession.getNotes() != null && !groomingSession.getNotes().trim().isEmpty()) { %>
                <div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                    <h4 class="text-sm font-medium text-slate-500 mb-2">Session Notes</h4>
                    <div class="text-sm whitespace-pre-wrap bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4">
<%= groomingSession.getNotes() %>
                    </div>
                </div>
                <% } %>
            </div>
        ...
        <div class="rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 p-10 text-center bg-white dark:bg-slate-900">
            <div class="mx-auto h-12 w-12 rounded-2xl bg-purple-50 dark:bg-slate-800 flex items-center justify-center mb-3">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
            </div>
            <h3 class="font-semibold">No grooming sessions yet</h3>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Grooming session records will appear here after grooming appointments.
            </p>
        </div>
        <% } %>
    </div>
